...
class OptPair_MinMaxDrawDown(OptPair_MaxSharpe):
    ...
    def objective(self, trial):
        ...
        if isinstance(daily_return, pandas.Series):
            daily_return_ = daily_return
            if self._look_back_start is not None:
                daily_return_ = daily_return_[daily_return_.index >= (
                            self._look_back_start + datetime.timedelta(days=1))]  # _look_back_start到_look_back_end 前开后闭
            if self._look_back_end is not None:
                daily_return_ = daily_return_[daily_return_.index <= self._look_back_end]
...
def run(each_pair0, config_file_yaml0):
    ...
    # print(opt_date_list)
    best_arg_path = r'E:/Strategy/MOMENTUM/best_args_MaxDrawDown'
...
if __name__ == '__main__':
    ...
    run_pairs_0 = ['A', 'AG', 'AL', 'AP', 'AU', 'B', 'BU', 'C', 'CF', 'CS', 'CU',
               'IC', 'IF', 'IH', 'JD', 'L', 'M', 'MA', 'NI', 'OI', 'P',
               'PB', 'PP', 'RM', 'RU', 'SC', 'SN', 'SR',
               'T', 'TA', 'TF', 'V', 'Y', 'ZN']  # 所有品种
    # run_pairs_0 = ['TA']
